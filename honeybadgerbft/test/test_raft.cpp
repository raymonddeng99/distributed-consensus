...
int main(){
    ...
    std::unordered_map<std::string, HoneyBadgerBFT::Client> cluster;
    for (const auto& nodeConfig: clusterConfig){
    	auto server = kj::heap<HoneyBadgerBFTServerImpl>(nodeConfig.ipAddress);
    	auto& serverRef = *server;
	    HoneyBadgerBFT::Client capability = kj::mv(server);

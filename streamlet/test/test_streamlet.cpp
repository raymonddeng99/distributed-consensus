...
int main(){
    ...
    std::unordered_map<std::string, Streamlet::Client> cluster;
    for (const auto& nodeConfig: clusterConfig){
    	auto server = kj::heap<StreamletServerImpl>(nodeConfig.ipAddress);
    	auto& serverRef = *server;
	    Streamlet::Client capability = kj::mv(server);
